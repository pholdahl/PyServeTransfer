...
MAX = 1                                     # Max number of clients at once

RESPCODES = {200: "OK", 404: "Not Found"}   # A dictionary containing responsecodes


# clientHandler function to handle a client
def clientHandler(client):                                                                          # Clienthandler function that takes in the client object
    try:
    ...
    requestMethod, requestPath, requestProtocol = incomingRequest[0].split()                        # Splitting up the message on index 0 GET http://host.com HTTP/1.1

    if "http" in requestPath:                                                                       # If the address contains http, it will also contain //
        url = requestPath.split("/", 3)[3]                                                          # And then the location after the third /
    else:                                                                                           
        url = requestPath.split("/", 1)[1]                                                          # Otherwise it only contains one /
...
<body>
    ...
    <p>The requested URL {url} was not found on this server</p>
        ...
        with open (url, 'rb') as file:                                                              # Trying to open the file and read as bytes
            content = file.read()                                                                   # Storing the data in content
...
def main():
    ...
    server.listen(MAX)                                                                                # Listening maximum number of clients, which is 1
    ...
    clientHandler(client)                                                                           # Passing client object to clientHandler
    server.close()                                                                                  # After serving 1 client, close the server
