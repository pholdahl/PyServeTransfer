...
# To run multiTester.py, comment the three lines using sys.argv, and uncomment the host, port and file variables in client.py

# Standard python start command
if __name__ == '__main__':
    st = time.time()                                                        # st, StartTime variable
    num = 1000                                                              # num, number of clients to run
    processes = []                                                          # Process list
    for i in range(num):                                                    # for loop 
        p = multiprocessing.Process(target=client.main())                   # p, what process to start
